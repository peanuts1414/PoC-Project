...
　上記の図３は、使用したモータ速度パターンを示すイメージ図になります。\
　パターンは2種類になります。\
...
　上記の図４は周期についてのイメージ図になります。周期一つにつきモータがワンサイクルしているイメージです。\
...
◆**2.4 異常データの生成について**◆\
 \
今回のデータでは、実機で異常データを出力するのはリスクがあったので、異常時に起きる現象を想定し生成いたしました。\
主に行ったことは、以下の3点になります。

- Scaleの変更：電流値の値を調整（例：scale=1.3 → 電流値を1.3倍する）
- ノイズ強度：ランダムな揺らぎを付与
- 突発ピーク：急激にスケールが大きくなる箇所の有無

これらの異常を付与することにより、機械に摩擦がある状態を想定してスケールを僅かにあげたり、\
...
本PoCでは、製造現場で使用されるモータの電流値、速度、位置情報を対象に異常検知を行いました。\
データは MT Developer2 のデジタルオシロを用いてCSV形式で取得しています。\
現状は手動でデータを収集し、前処理用のスクリプトに渡す形となっています。

### .2 課題

手動でのデータ収集では以下の制約があります：
- 作業者がデータ収集を行う必要があり、作業コストがかかる
- データ取得のタイミングや周期が一定でない場合があり、モデル学習用データの精度に影響する
- 長期運用やリアルタイム異常検知を行う際に自動化が必須

### 3.3 自動化に向けた検討

PoCを運用段階に移行するためには、以下の点を検討する必要があります：
1. PLCとモデルの連携
    - モータやセンサからのデータをPLCで取得
    - 取得データをリアルタイムでモデルに入力するフローを構築

2. データ収集の自動化
    - ラズベリーパイや産業用IoTゲートウェイを用いてデータを自動取得
    - 定期的にCSVやデータベースに保存する仕組みを構築

3. 同期と前処理
    - モータのワンサイクル単位でのウィンドウ分割や特徴量生成を自動化
    - 停止・起動時のデータを除外し、モデル学習用データを安定的に生成

### 3.4 今後の課題

- 運用に適したデータ取得ハードウェアの選定（PLC接続、IoTゲートウェイ、センサインタフェース）
- データ同期の精度向上（高速サンプリングやタイムスタンプ管理）
- 自動化スクリプトやバッチ処理の実装による運用コスト削減
- 収集データのモニタリングおよび異常検知結果のフィードバックループ構築
...
いずれも教師なし学習に基づくアプローチであり、ラベル付き異常データが限られている製造現場の課題に適している。\
 ...
 ◆**Isolation Forest**◆\
  ...
  Isolation Forestは計算が高速で、大規模データに適しています。また、実装コストも抑えられる点も他のモデルより優れています。\
 \
 \
 ◆**One-Class SVM**◆\
  \
...
　　　　　<ins>その場合は特にRecall指標を注意してスコアを高めていく必要があります。</ins>\
 \
今回のPoCは製造ラインでの現場を想定しており、見逃し（Recallの低さ）は重大なリスクにつながるため、Recallを特に重視しました。\
また、誤検知（Precisionの低さ）もライン停止に直結するため、PrecisionとのバランスをF1-scoreで確認しました。\
手法として、異常値をなぜ検知できなかったのか突き詰めるため異常データを可視化して正常データと比較しました。\
　\
今回の失敗例を挙げると、上手く異常検知できていない箇所があったが理由がわからないということがありました。\
そのときに各出力データを可視化して確認すると、異常判定する以前に大きな問題があったためモデルのしきい値などの調整では解決できないという失敗がありました。\
そのため、あらためてデータの可視化が重要な手法であるということを認識しました。
...
図５：評価結果グラフ
 \
Accuracy, Precision, Recall, F1-scoreどのモデルも高い精度を確認でき、製造データを対象とした異常検知の有効性が示されました。\
また、図５のAutoencoderグラフを確認すると、特にAutoencoderでは正常データと異常データの分布が明確に分離されており、異常検知制度の高さが視認できます。\
 \
**◆ モデルごとの特徴 ◆**
- **Isolation Forest**：シンプルな構造で実装・解釈が容易。スコア面では他モデルに劣るものの、学習コストが低く、実運用での拡張性が高い。
- **One-Class-SVM**：Accuracy, Precision, F1-scoreにおいて他モデル上回り、今回のような少量データでの異常検知には高い精度を出すことが可能。ただし、データ量が増えると計算コストが急増するため、大規模運用には課題が残る。
- **Autoencoder**：正常と異常を明確に分離でき、高い精度を示した。ただし、十分な学習データと学習時間が必要であり、PoC規模では過学習の懸念がある。

### 8.2考察とまとめ
**◆ 考察(得られた知見・制約・注意点) ◆**\
 \
本PoCの目的は「製造データを対象とした異常検知のPoC構築」であり、テーマの中心はIsolation Forestにあります。結果だけを見ればOne-Class-SVMが優秀でしたが、実装の容易さ、学習コストの低さ、運用現場での扱いやすさを考慮すると、Isolation Forestは依然として最有力候補です。\
一方で、AutoencoderやOne-Class-SVMは特定の条件（データ量の少なさ、複雑な非線形性など）で有効性が確認でき、今後の比較対象として価値があります。\
  \
 **◆ まとめ ◆**  
スコアの面ではOne-Class-SVMが最も高い結果を示しましたが、PoCのテーマおよび実運用を見据えた観点からは、Isolation Forestを主要な異常検知手法として選定することが妥当と考えます。

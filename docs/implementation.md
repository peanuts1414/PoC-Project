...
また、図５のAutoencoderグラフを確認すると、特にAutoencoderでは正常データと異常データの分布が明確に分離されており、異常検知制度の高さが視認できます。\
 \
**◆ モデルごとの特徴 ◆**\
- **Isolation Forest**：全体的にバランスが良く、シンプルに実装できる点が強み
- **One-Class-SVM**：Accuracy, Precision, F1-scoreにおいて他モデル上回り、今回のような少量データでの異常検知には高い精度を出すことが可能
- **Autoencoder**：正常と異常を明確に分離でき、高い精度を出すことが強み。ただし、汎用性を高めるには大量の学習データと学習時間などの学習コストが高くつく

**◆ 考察（得られた知見、制約、注意点） ◆**\
 \
 本PoCでは図５からAutoencoderが最も正常と異常の分離性能を示していることが確認できます。
 しかし、それは学習データの少なさから過学習を起こしている可能性も考えられ、実際に実装を検討をする場合にはさらなる検証が必要になります。

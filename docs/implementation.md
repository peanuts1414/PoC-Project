...
- データ量の拡大
    - 現状の学習データは比較的少量であり、特にAutoencoderは過学習のリスクが存在します。
    - 実運用を想定する場合、運転中の異常データを含めた大量データの収集が必要です。

- データバリエーションの拡充
    - モータの速度パターンや負荷条件のバリエーションを増やすことで、モデルの汎用性と精度向上が期待できます。

- データ前処理の自動化
    - 現状はワンサイクル単位のウィンドウ分割や特徴量生成が手作業または手動スクリプト依存です。
    - 運用段階では、収集から前処理まで自動化できる仕組みを構築する必要があります。

### 9.2 モデル関連の課題

- モデル選定と最適化
    - 本PoCではIsolation Forestをテーマにしていますが、One-Class-SVMがスコア的に最良となりました。
    - 実運用では、モデル選定や閾値設定の自動最適化を検討する必要があります。

- 閾値設定の自動化
    - 現状は閾値を手作業で調整していますが、実運用に向けて精度と漏れを最適化する自動調整手法が必要です。

### 9.3 PLCおよびデータ収集連携

- リアルタイムデータ取得
    - MT Developer2によるCSV取得はPoC用であり、実運用ではPLCやIoT機器と連携してデータをリアルタイムで取得する必要があります。

- データ同期と整形
    - ワンサイクル単位でのウィンドウ分割や特徴量生成を運用フローに組み込み、停止・起動データの除外や異常データのラベル付けを自動化する必要があります。

- 機材選定
    - Raspberry Piや産業用IoTゲートウェイの導入により、PLCからのデータ収集を自動化する方法を検討することが望ましいです。

### 9.4 運用・検証の課題

- モデル精度の長期検証
    - 学習データや運転条件が変化した場合のモデル性能維持を確認するため、定期的な再学習や精度評価が必要です。

- フィードバックループの構築
    - 異常検知結果を現場運用に反映する仕組みや、検出結果のレビュー・改善プロセスを設計する必要があります。
